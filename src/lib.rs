...
pub fn run_cli() {
    ...
    let results = run(&tree, &access_key, "MESSAGE", Some(&fields), Some(".")).unwrap();
    fs::write("results.json", serde_json::to_string(&results).unwrap()).expect("Error writing results file");
}
